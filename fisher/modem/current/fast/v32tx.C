...
static float shapetab[2 * SYMBLEN + 1] = {
    /* Raised cosine pulse shaping with Beta = 0.5; square-root, with x/sinx
       compensation */
    +1.1510798492, +0.9848450677, +0.5795790021, +0.1495226763, -0.1156411181,
    -0.1625782395, -0.0750914609, +0.0185567369, +0.0448950047,
};
...
inline void pbit(int b) {
  callco(tx2_co, b);
}
inline int gbit() {
  return callco(tx1_co);
}
...
global void inittx() {
  ...
  if (rateword & rb_14400)
    rateword &= ~(rb_12000 | rb_9600 | rb_7200 | rb_4800);
  else if (rateword & rb_12000)
    rateword &= ~(rb_9600 | rb_7200 | rb_4800);
  else if (rateword & rb_9600)
    rateword &= ~(rb_7200 | rb_4800);
  else if (rateword & rb_7200)
    rateword &= ~rb_4800;
  else if (rateword & rb_4800)
    ;
  else
    giveup("can't agree on a speed!");
  infomsg(">>> E2: rates = %04x", rateword); /* send E2 */
  sendrate(rateword);
  enc->setrate(rateword); /* tell encoder what bit rate to use */
  while (mstate < 6)
    pbit(1); /* sync to Rx side */
  for (int i = 0; i < 128; i++)
    pbit(1); /* followed by 128 "1" bits */
}
...
static void tx2_loop() { /* round-trip-delay estimation */
  while (mstate < 2) {
    complex z = (mstate == 0) ? ztab2[0] : ztab2[3]; /* A or C */
    sendsymbol(z);
    sendsymbol(z); /* AA or CC */
  }
  /* train equalizer */
  while (mstate == 2)
    sendsymbol(0.0);
  ...
  for (int bc = SEG_1; bc < SEG_3 + TX_TRNLEN; bc++)
    sendsymbol(trn->get(bc));
  mstate++; /* from 3 to 4 */
  /* exchange data */
  enc->reset();
  for (;;) {
