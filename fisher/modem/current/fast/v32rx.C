...
global void initrx() {
  ...
  for (int i = 0; i < 128; i++)
    gbit();    /* discard 128 "1" bits (wait for trellis decoder to settle) */
...
static ushort getrate() {
  ushort wd = getrwd();
l:
  until((wd & 0xf111) == 0x0111) wd = (wd << 1) | gbit();
  ushort rate = wd;
...
static void rx2_loop() { /* round-trip-delay calculation */
  carrier->resetphase();
  roundtrip();
  /* train equalizer */
  carrier->resetphase();
...
static void wt_tone(int k0,
                    int k1,
                    int max,
                    bool chk) { /* wait for a stable tone */
...
static int wt_reversal(int k0, int k1) { /* wait for a phase reversal */
  complex z0 = ztab2[k0], z1 = ztab2[k1];
  int bc = 0;
  int t;
  bool rev = false;
...
static void traincanceller() { /* train canceller at half-symbol intervals */
  while (mstate == 3) {
    complex yz = gethalfsymb();
    can->update(yz);
