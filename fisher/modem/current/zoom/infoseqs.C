...
static float _fstepf_1(
    filter* fi,
    float x) { /* /usr/fisher/mipsbin/mkfilter -Bu -Lp -o 4 -a 0.03125 -l */
  float* v = fi->v;
  v[0] = v[1];
  v[1] = v[2];
  v[2] = v[3];
  v[3] = v[4];
  v[4] = (7.2772549288e-05 * x) + (-0.5980652616 * v[0]) +
         (2.6988843913 * v[1]) + (-4.5892912321 * v[2]) + (3.4873077415 * v[3]);
...
static void tidydebug() {
  ...
  for (int k = 0; k < debugptr; k++)
    putc(debugbuf[k], stderr);
  fprintf(stderr, "\r\n");
}
...
static void getranging() {
  ...
  do
    ack = getinfo();
  until(ack);             /* keep looking for an info seq with ack bit set */
  mstate++;               /* 0 to 1 */
...
static complex gethalfsymb() { /* sample at half-symbol intervals */
  complex yz;
...
static void getprobing() {
  for (int i = 0; i < SECS(0.18); i++)
    insample(); /* discard 10 ms A + 160 ms L1 + 10 ms */
  goertzel** gvec = new goertzel*[25];
  for (int i = 0; i < 25; i++)
    gvec[i] = new goertzel((i + 1) * 150.0);
