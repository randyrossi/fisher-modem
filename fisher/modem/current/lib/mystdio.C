...
global void openstdio() {
  ...
  rmode.c_cc[VMIN] = 1;
  rmode.c_cc[VTIME] = 0; /* set timeout params  */
...
global int my_getchar() { /* get char from stdin */
  return inbuffer.getch();
}

int Buffer::getch() {
  return ((head - tail) > 0) ? buf[tail++ & BUFFERMASK] & 0xff : eof ? EOF
                                                                     : NOCHAR;
}

global void my_putchar(int ch) { /* put char to stdout */
  outbuffer.putch(ch);
}
...
static void clockhandler(int sig) {
  ...
  if (timeout > 0 && --timeout == 0)
    kill(getpid(), SIGUSR1); /* "Remote modem is not responding" */
  inbuffer.fill();
  outbuffer.empty();
}
