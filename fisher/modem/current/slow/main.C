...
static vminfo vmodes[] = {
    ...
    {"-V32o", V32o}, {"-V34o", V34o}, {NULL, (vmode)0}, /* terminator */
};
...
global void main(int argc, char** argv) {
  ...
  setoptions(argv);
  checkoptions();
  if ((options & opt_org) && !legaltelno(telno))
    giveup("Illegal telephone number");
  if (options & opt_mod) {
    uint b = legalbps(veemode);
    unless(options & opt_bps) bitrates = b; /* default is the full set */
    if (bitrates & ~b)
      giveup("illegal mode-bitrate combination");
  }
  if (options & opt_fax) {
    initdoc();
    if (options & opt_org)
      readdoc(); /* read whole doc before time-critical part */
  }
  openaudio();
  atexit(closeaudio);
  seizeline();
  ...
  if (options & (opt_fax | opt_mod)) {
    ...
    if (options & opt_ans)
      sendanswer(); /* send answer tone */
    int contime = time(NULL);
    infomsg("Connected");
    if (options & opt_v8)
      startsession(); /* V.8 procedures */
    if (options & opt_fax)
      becomefax();
    if (options & opt_mod)
      becomemodem();
  ...
  if ((options & opt_fax) && !(options & opt_org))
    writedoc(); /* write whole doc after time-critical part */
  exit(0);
}
  ...
  until(argv[ap] == NULL) {
    ...
    if (s[0] == '+' || s[0] == '=') {
      ...
      if (s[k] == 'V') {
        ...
        options |= opt_mod;
        if (veemode == V34o)
          options |= opt_v8; /* V.34 requires V.8 */
      } else {
        until(s[k] == '\0') {
          char ch = s[k++];
          if (ch == 'v')
...
static uint bitrate(int n) { /* see bitrates.h */
  unless(n >= 2400 && n <= 28800 && n % 2400 == 0)
      giveup("unknown bit rate %d", n);
  return 1 << (n / 2400 - 1);
}
...
static void checkoptions() {
  ...
  unless(~options & (opt_org | opt_ans)) usage(7); /* but not both
                                                      */
  unless(~options & (opt_fax | opt_mod))
      usage(8); /* mustn't have both fax and mod */
}

static void usage(int n) {
  fprintf(stderr, "Modem Vsn. 5.1 from <fisher@minster.york.ac.uk>\n");
  fprintf(stderr,
          "Usage(%d): modem [-v] [-V8] {-<mode> | -fax} [-bps <bps> ...] "
          "[+<telnum> | -ans]\n",
          n);
  fprintf(stderr, "Modes:");
  for (int i = 0; vmodes[i].s != NULL; i++)
    fprintf(stderr, " %s", vmodes[i].s);
  putc('\n', stderr);
  exit(2);
}
