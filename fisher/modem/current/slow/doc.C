...
global void initdoc() {
  g3file = tmpfile();
  if (g3file == NULL)
    giveup("can't create temporary file");
}

global void readdoc() { /* Copy document from stdin to g3file */
  infomsg("Reading input...");
  int vsn, xres, yres;
  int ni = scanf("!<Group 3> %d %d %d\n", &vsn, &xres, &yres);
  unless(ni == 3 && vsn == VERSION && xres == 200 &&
         (yres == 100 || yres == 200)) giveup("input is not in Group 3 format");
  if (yres == 200)
    options |= opt_H;
  numpages = 0;
  int ch = getchar();
...
l:
  if (ch >= 0)
    giveup("sorry, too many pages");
  if (fflush(g3file) != 0)
    giveup("write error on temp file");
}

global void sendpage(int pn,
                     int scanbits) { /* Copy one page from g3file to V.29 */
  if (options & opt_v)
    fprintf(stderr, ">>> Page %d\n", pn);
  if (pn < 1 || pn > numpages)
    giveup("bug: bad page page number: %d", pn);
  ...
  while (nb < pagebits[pn - 1]) {
    ...
    for (int j = 0; j < 8 && nb < pagebits[pn - 1]; j++) {
      ...
      {
        while (lb++ < scanbits)
          putbit(0); /* pad scan line to achieve min. scan time */
        lb = 0;
      }
      putbit(b);
      lb++;
  ...
  for (int i = 0; i < 50; i++)
    putbit(0); /* flush buffer */
}

global void receivepage(int pn) { /* Copy one page from V.29 to g3file */
  if (options & opt_v)
    fprintf(stderr, "<<< Page %d\n", pn);
  if (pn < 1)
    giveup("bug: bad page page number: %d", pn);
...
global void writedoc() { /* Copy document from g3file to stdout */
