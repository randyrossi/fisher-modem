...
static void senddoc() {
  ...
  unless(frame[0] == DIS && framelen >= 4)
      giveup("Failed to get a valid DIS frame");
  ...
  if (pagecount < numpages)
    giveup("Document transmission failed");
  infomsg("Success - delivered %d pages", pagecount);
}

static void makedcs(uchar* disfr) { /* make DCS frame from received DIS;
  ...
  infomsg("resolution=%c; speeds=%02x; scanbits=%d",
          (options & opt_H) ? 'H' : 'L', spd, scanbits);
...
static void receivedoc() {
  ...
  do {
    ...
    unless(frame[0] == EOP || frame[0] == MPS)
        giveup("Remote fax has given up");
    more = (frame[0] == MPS);
...
static void getdcs(bool first) {
  ...
  unless(frame[0] == DCS && framelen >= 4)
      giveup("Failed to get a valid DCS frame");
  checkdcs(frame);
}

static void checkdcs(uchar* dcsfr) { /* check DCS frame from remote sender */
...
static void makeident(uchar cmd) {
  ...
  while (len > 0)
    identframe[p++] = shuffle[MYNAME[--len]];
  while (p < 21)
...
static ushort computecsum(uchar* msg, int len) { /* CCITT V.41 crc (sort of) */
  ushort reg = 0xffff;
  while (len-- > 0) {
    uchar x = *(msg++);
    for (int j = 0; j < 8; j++) {
...
static void setstate(int st) {
  unless(st == state) {
    switch (st) {
      case RX_CTL_STATE:
        flushoutput();
        discardinput(); /* flush Tx before switching to Rx */
        initrx_fsk(V21o);
        break;

      case TX_CTL_STATE:
        if (state == TX_DOC_STATE)
          sendpause(0.075); /* pause before switching from TX_DOC to TX_CTL */
        inittx_fsk(V21a);
        for (int i = 0; i < 40; i++)
          putsync(HDLC_FLAG); /* send HDLC preamble */
        break;
