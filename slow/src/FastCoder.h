...
#define FAST_SEG_1 0                  /* ABAB...	 */
#define FAST_SEG_2 (FAST_SEG_1 + 256) /* CDCD...	 */
#define FAST_SEG_3 (FAST_SEG_2 + 16)  /* TRN (A/C)	 */
#define FAST_SEG_4 (FAST_SEG_3 + 256) /* TRN (A/B/C/D) */

struct FastTrainingGen {
  FastTrainingGen(scrambler* s) { scr = s; }
  complex get(int);
...
struct FastDecoder {
  ...
  void reset() {
    setrate(rb_4800);
    q = 0;
  } /* no need to re-init trellis; we haven't used it yet */
