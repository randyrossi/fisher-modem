...
static uchar disframe[] = {
    DIS, 0x00, 0xe2,
    0x0e}; /* V.29 only; no 2d coding; set "fine" bit; no scan-line padding */
...
void Fax::senddoc() {
  ...
  until((frame[0] == DIS && framelen >= 4) || ntries >= 14);
  unless(frame[0] == DIS && framelen >= 4)
      modem->giveup("Failed to get a valid DIS frame");
  ...
  while (pagecount < numpages) {
    ...
    ntries = 0;
    do {
      sendframe((pagecount + 1 < numpages) ? mpsframe : eopframe, 1,
                true); /* send MPS or EOP */
    ...
    if (frame[0] == MCF || frame[0] == RTP)
      pagecount++;
    if (frame[0] == RTP || frame[0] == RTN)
      sendtraining();
  }
  sendframe(dcnframe, 1, true); /* send DCN */
  if (pagecount < numpages)
    modem->giveup("Document transmission failed");
  modem->infomsg("Success - delivered %d pages", pagecount);
}

void Fax::makedcs(
    uchar*
        disfr) { /* make DCS frame from received DIS; establish capabilities */
  dcsframe[0] = DCS;
  dcsframe[1] = 0x00;
  unless(disfr[2] & 0x40) modem->giveup("Remote fax can't do Group 3 (T.4)");
  uchar spd = (disfr[2] >> 2) & 0xf;
  dcsframe[2] =
      0x70; /* set to V.29 7200 bit/s; no 2d coding; clear "fine" bit */
  uchar n = (disfr[3] >> 1) & 7; /* DIS bits 21-23 */
  if (modem->modemOptions->options & opt_H) {
    unless(disfr[2] & 0x02)
        modem->giveup("Remote fax can't do 200 dpi; try -l");
    dcsframe[2] |= 0x02; /* ask for high density */
    n += 8;
  }
  n = dcstranstab[n];   /* translate to DCS equivalent */
  dcsframe[3] = n << 1; /* set A4 1728 dots per scan line; set scantime bits;
                           clear extend bit */
  scanbits = scanbitstab[n]; /* min. scan time, in bits */
  modem->infomsg("resolution=%c; speeds=%02x; scanbits=%d",
                 (modem->modemOptions->options & opt_H) ? 'H' : 'L', spd,
                 scanbits);
  unless(spd & 0x8) modem->giveup("Remote fax can't do V.29");
}

void Fax::sendtraining() {
  int ntries = 0;
  do {
    ...
    for (int j = 0; j < 10800; j++)
      modem->v29->putbit(0); /* TCF, 1.5 sec @ 7200 bps */
    getmessage();
    ntries++;
  }
  until(frame[0] == CFR || ntries >= 3);
  unless(frame[0] == CFR) modem->giveup("Remote fax training failed");
}
...
void Fax::receivedoc() {
  ...
  unless(ok) modem->giveup("Local fax training failed");
  ...
  do {
    ...
    unless(frame[0] == EOP || frame[0] == MPS)
        modem->giveup("Remote fax has given up");
    more = (frame[0] == MPS);
    sendframe(mcfframe, 1, true);
    pagecount++;
  } while (more);
  getmessage(); /* should be DCN */  // ???
  modem->infomsg("Success - received %d pages", pagecount);
}

void Fax::getdcs(bool first) {
  int ntries = 0;
  do {
    if (first) {
      ...
      sendframe(fttframe, 1, true); /* send FTT */
    getmessage();                   /* try for DCS */
    ntries++;
  }
  until((frame[0] == DCS && framelen >= 4) || frame[0] == DCN || ntries >= 6);
  unless(frame[0] == DCS && framelen >= 4)
      modem->giveup("Failed to get a valid DCS frame");
  checkdcs(frame);
}

void Fax::checkdcs(uchar* dcsfr) { /* check DCS frame from remote sender */
  ...
  modem->infomsg("resolution=%c; speeds=%02x; scanbits=%d",
                 (modem->modemOptions->options & opt_H) ? 'H' : 'L', spd,
                 scanbits);
  unless(dcsfr[2] & 0x40) modem->giveup("Remote fax can't do Group 3 (T.4)");
  unless(spd & 0x8) modem->giveup("Remote fax can't do V.29");
}

bool Fax::gettraining() {
  setstate(RX_DOC_STATE);
  int totnz = 0, tot = 0;
  for (int j = 0; j < 15; j++) {
    int nz = 0;
...
void Fax::makeident(uchar cmd) {
  int len = strlen(MYNAME);
  int p = 0;
  identframe[p++] = cmd; /* TSI or CSI */
  while (len > 0)
    identframe[p++] = shuffle[(uchar)MYNAME[--len]];
  while (p < 21)
    identframe[p++] = 0x04; /* pad with spaces */
}
...
void Fax::getmessage() {
  ...
  isfinal = true;
  msgendtime = samplingDevice->samplecount +
               4 * SAMPLERATE; /* reset timeout counter (4 secs in future) */
  nextval();                   /* read 1st byte of preamble */
  do {
    getmsg();
    if (frame[0] == CSI || frame[0] == TSI)
      printident();
  }
...
void Fax::printident() {
  char buf[21];
  int p = framelen, k = 0;
  while (p > 1 && frame[p - 1] == 0x04)
    p--; /* delete spaces */
  while (p > 1 && k < 20)
    buf[k++] = shuffle[frame[--p]];
  buf[k++] = '\0';
  modem->infomsg("Remote fax identifies as: %s", buf);
}
...
void Fax::getmsg() {
  ...
  until(ok || rxval == HDLC_TIMEOUT) {
    ...
    {
      msglen = 0;
      until(rxval == HDLC_FLAG || rxval == HDLC_TIMEOUT ||
            msglen >= MAXMESSAGE) {
      ...
      unless(rxval == HDLC_TIMEOUT) {
        if (rxval != HDLC_FLAG)
          msgerror("Message too long: %02x %02x %02x...", message[0],
                   message[1], message[2]);
        else if (msglen < 5) /* ensure frame length .ge. 1 */
          msgerror("Message too short");
        else
          unless(csumok(message, msglen)) msgerror("Checksum error");
...
void Fax::nextval() {
  rxval = (after(samplingDevice->samplecount, msgendtime))
              ? HDLC_TIMEOUT
              : modem->fsk->getsync();
}

void Fax::msgerror(char* msg, word p1, word p2, word p3) {
  if (modem->modemOptions->options & opt_v) {
...
void Fax::sendframe(uchar* fr, int frlen, bool final) {
  setstate(TX_CTL_STATE);
  printframe(">>>", fr, frlen);
  uchar message[MAXMESSAGE];
  message[0] = 0xff;
  message[1] = final ? 0xc8 : 0xc0;
  memcpy(&message[2], fr, frlen);
  ushort csum = computecsum(message, frlen + 2);
  message[frlen + 2] = (csum >> 8) ^ 0xff;
  message[frlen + 3] = (csum & 0xff) ^ 0xff;
  unless(csumok(message, frlen + 4)) modem->giveup("Bug! Tx checksum");
  modem->fsk->putsync(HDLC_FLAG);
  modem->fsk->putsync(HDLC_FLAG);
  for (int i = 0; i < frlen + 4; i++) {
    modem->fsk->putsync(message[i]);
  }
  modem->fsk->putsync(HDLC_FLAG);
}

void Fax::printframe(char* io, uchar* fr, int frlen) {
  if (modem->modemOptions->options & opt_v) {
...
bool Fax::csumok(uchar* msg, int len) {
  ushort csum = computecsum(msg, len);
  return (csum == 0x1d0f);
}

ushort Fax::computecsum(uchar* msg, int len) { /* CCITT V.41 crc (sort of) */
  ushort reg = 0xffff;
  while (len-- > 0) {
    uchar x = *(msg++);
    for (int j = 0; j < 8; j++) {
...
void Fax::setstate(int st) {
  unless(st == state) {
    switch (st) {
      case RX_CTL_STATE:
        // flushoutput();
        // samplingDevice->discardInput(); /* flush Tx before switching to Rx */
        helper->suspendReading();
        helper->resumeWriting();
        modem->fsk->initrx_fsk(ModemOptions::V21o);
        break;

      case TX_CTL_STATE:
        helper->suspendWriting();
        helper->resumeReading();
        if (state == TX_DOC_STATE) {
          modem->sendpause(
              0.075); /* pause before switching from TX_DOC to TX_CTL */
        }
        modem->fsk->inittx_fsk(ModemOptions::V21a);
        for (int i = 0; i < 40; i++)
          modem->fsk->putsync(HDLC_FLAG); /* send HDLC preamble */
        break;

      case RX_DOC_STATE:
        printf("******* TO RX_DOC STATE\n\r");
        helper->suspendReading();
        helper->resumeWriting();
      ...
      case TX_DOC_STATE:
        printf("******* TO TX_DOC STATE\n\r");
        helper->suspendWriting();
        helper->resumeReading();
        modem->sendpause(
            0.075); /* pause before switching from anything to TX_DOC */
        printf("******* init tx v29\n\r");
        modem->v29->inittx_v29();
        break;
    }
    state = st;
  }
