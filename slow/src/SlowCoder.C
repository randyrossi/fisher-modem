...
complex SlowTrainingGen::get(int bc) { /* V.29 training sequence */
  complex z;
  if (bc < SLOW_SEG_2) { /* segment 1 : silence */
    z = 0.0;
  } else {
    int k;
    if (bc < SLOW_SEG_3) { /* segment 2 : ABAB... */
      k = (bc & 1) ? 7 : 4;
    } else { /* segment 3 : CDCD... */
      int b6 = (reg >> 1) & 1;
      int b7 = reg & 1;
...
complex SlowEncoder::encode(int bits) {
  state += fwd_phinctab[bits & 7];
  state &= 7;
  return constellation[state];
}

int SlowDecoder::decode(complex z) {
  int nst = locate(z);
...
int SlowDecoder::locate(complex z) {
  int k;
...
complex SlowDecoder::getez() {
  return constellation
      [state]; /* return exact (quantized) value of last decoded symbol */
}
