...
complex FastTrainingGen::get(int bc) {
  int k;
  if (bc < FAST_SEG_2) { /* segment 1 : ABAB... */
    k = (bc & 1) ? 1 : 0;
  } else if (bc < FAST_SEG_3) { /* segment 2 : CDCD... */
    k = (bc & 1) ? 2 : 3;
  } else { /* segment 3 & 4 : scrambled sequence */
    int b1 = scr->fwd(1);
    int b2 = scr->fwd(1);
    if (bc < FAST_SEG_4)
      b2 = b1;
...
void FastEncoder::setrate(ushort rw) {
  rate = rw & (rb_4800 | rb_7200 | rb_9600 | rb_12000 | rb_14400);
  t = 0;
}

complex FastEncoder::encode(int b) {
  switch (rate) {
    default:
      fprintf(stderr, "can't encode rate %04x", rate);
...
void FastDecoder::inittrellis() /* private */
{
...
void FastDecoder::printtrellis(char* fn) {
  FILE* fi = fopen(fn, "w");
  if (fi == NULL) {
...
void FastDecoder::setrate(ushort rw) {
  rate = rw & (rb_4800 | rb_7200 | rb_9600 | rb_12000 | rb_14400);
}

int FastDecoder::decode(complex z) {
  switch (rate) {
    default:
      fprintf(stderr, "can't decode rate %04x", rate);
    ...
    case rb_7200: {
      ...
      return (td_difftab[q & 0xf] << 1) |
             (r & 1); /* diff. decode & return result */
    }
  }
}

int FastDecoder::viterbi(complex z) /* private */
{
  int ncol = (tindex + 1) % TLEN; /* index of next column */
  float metric[NSTATES];          /* dist^2 from z to closest point in j'th
                                     sub-constellation */
  ...
  for (int j = 0; j < NSTATES; j++) {
    ...
    tnode* x = &trellis[tindex][j];
    float svlen =
        (x->svlen) * 0.99; /* length of survivor from x, reduced by a bit to
                              ensure stability */
    for (int k = 0; k < NINPUTS; k++) {
  ...
  return (x->path >> (TLEN - 1) * 3) &
         7; /* return bits about to drop off lh end */
}

static uchar ctab4[] = {
    0xf, 0x1, 0x3, 0xd, 0x5, 0x8, 0xa, 0x7,
    0x6, 0xb, 0x9, 0x4, 0xc, 0x2, 0x0, 0xe,
};

complex FastDecoder::getez() {
  switch (rate) {
    default:
      fprintf(stderr, "can't decode rate %04x", rate);
