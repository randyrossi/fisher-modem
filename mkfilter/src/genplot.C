...
static void
draw_xtick(gdImagePtr, double, double, int, int, char*, double = 0.0);
static void
draw_ytick(gdImagePtr, double, double, int, int, char*, double = 0.0);
...
static void usage() {
  ...
  fprintf(stderr,
          "Usage: genplot [-i <n> | -s <n> -a <a1> <a2> | -l] [-log <min>] "
          "[-d] out.gif\n");
  exit(1);
}
...
static void drawgraph() {
  switch (gtype) {
    ...
    case locus: {
      ...
      unless(ok) fprintf(
          stderr, "genplot: warning: step length %g in locus plot\n", stepl);
      break;
    }
...
static void locusgraph(char* fn, complex fr[], int nsteps, double& stepl) {
  ...
  gdImageLine(im, xmap_lc(-1.0), ymap_lc(0.0), xmap_lc(+1.0), ymap_lc(0.0),
              black); /* X axis	 */
  gdImageLine(im, xmap_lc(0.0), ymap_lc(-1.0), xmap_lc(0.0), ymap_lc(+1.0),
              black); /* Y axis	 */
  for (int i = 1; i <= nsteps; i++) {
    complex z0 = fr[i - 1], z1 = fr[i];
    gdImageLine(im, xmap_lc(z0.re), ymap_lc(z0.im), xmap_lc(z1.re),
                ymap_lc(z1.im), red);
...
static void frgraph(char* fn, complex fr[], int nsteps) {
  ...
  gdImageLine(im, xmap_pm(0.0), ymap_pm(0.0), xmap_pm(1.0), ymap_pm(0.0),
              black); /* X axis	    */
  gdImageLine(im, xmap_pm(0.0), ymap_pm(0.0), xmap_pm(0.0), ymap_pm(1.0),
              black); /* left Y axis  */
  gdImageLine(im, xmap_pm(1.0), ymap_pm(0.0), xmap_pm(1.0), ymap_pm(1.0),
              black); /* right Y axis */
  for (i = 0; i <= nsteps; i++) {
    ymvals[i] = hypot(fr[i]);
    ypvals[i] = (ymvals[i] > PHEPS)
                    ? atan2(fr[i]) / TWOPI + 0.5
                    : -1.0; /* out-of-range value will not be plotted */
  }
  /* ymvals, ypvals in range 0 .. 1 */
  if (logopt) {
    for (i = 0; i <= nsteps; i++)
      ymvals[i] = (ymvals[i] > 0.0)
                      ? (20.0 * log10(ymvals[i]) - logmin) / (-logmin)
                      : -1.0; /* out-of-range value will not be plotted */
  }
  ...
  if (logopt) {
    ...
    for (i = 0; i <= 10; i++)
      draw_ytick(im, 0.0, 0.1 * i, red, -TICKLEN, fmt,
                 (1.0 - 0.1 * i) * logmin);
  } else {
    for (i = 0; i <= 10; i++)
...
static void pgraph(gdImagePtr im,
                   double vec[],
                   int nsteps,
                   int col,
                   bool wrapy) {
  for (int i = 1; i <= nsteps; i++) {
    ...
    {
      if (wrapy && y0 - y1 > 0.5) {
        gdImageLine(im, xmap_pm(x0), ymap_pm(y0), xmap_pm(x1), ymap_pm(1.0),
                    col);
        gdImageLine(im, xmap_pm(x0), ymap_pm(0.0), xmap_pm(x1), ymap_pm(y1),
                    col);
      } else if (wrapy && y1 - y0 > 0.5) {
        gdImageLine(im, xmap_pm(x0), ymap_pm(y0), xmap_pm(x1), ymap_pm(0.0),
                    col);
        gdImageLine(im, xmap_pm(x0), ymap_pm(1.0), xmap_pm(x1), ymap_pm(y1),
                    col);
      } else
        gdImageLine(im, xmap_pm(x0), ymap_pm(y0), xmap_pm(x1), ymap_pm(y1),
                    col);
    }
  }
}

static void draw_xtick(gdImagePtr im,
                       double x,
                       double y,
                       int col,
                       int tlen,
...
static void draw_ytick(gdImagePtr im,
                       double x,
                       double y,
                       int col,
                       int tlen,
  ...
  if (tlen < 0)
    gdImageString(im, gdFontSmall, ix + 2 * tlen - swid, iy - shgt / 2, str,
                  col);
  if (tlen > 0)
    gdImageString(im, gdFontSmall, ix + 2 * tlen, iy - shgt / 2, str, col);
}
...
static char* choosefmt(double a1,
                       double a2) { /* choose smallest format such that all
                                       ticks are labelled distinctly */
  static char fmt[10];
  bool ok;
  int p = 0;
  do {
