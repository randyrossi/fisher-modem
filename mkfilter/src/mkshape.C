...
#define opt_t 0x100 /* output 2nd half shapetab for tx source files */
...
static void usage() {
  ...
  fprintf(stderr,
          "Usage: mkshape -{cr} <alpha> <beta> <len> [-{lwx}] [-b nb]\n");
...
static void apply_window() /* apply Hamming window to impulse response */
{
  if (options & opt_w) {
    ...
    for (int i = 1; i <= h; i++) {
      double w =
          0.54 - 0.46 * cos(TWOPI * (double)(h + i) / (double)(numcoeffs - 1));
      xcoeffs[h + i] *= w;
      xcoeffs[h - i] *= w;
    }
  }
...
static void trunc_coeffs() {
  if (options & opt_b) {
    ...
    int h = (numcoeffs - 1) / 2;
    double max =
        (options & opt_h) ? xcoeffs[h - 1] : xcoeffs[h]; /* max coeff */
...
static void printresults(char** argv) {
  if (options & opt_t) {
    // Comment the command line since this is code
    printf("//");
  }
  printcmdline(argv);
  double gain = 0.0;
  ...
  // opt_t was added to help make the shape tables used by v32tx and
  // v34tx dynamic since they are ultimately tied to the sample rate
  // This option prints source code that represents the 2nd half of the
  // raised cosine shape since that's what those tx functions want
  if (options & opt_t) {
    printf(
        "// DO NOT EDIT - This code automatically generated with mkshape.C\n");
    printf("// Note: This is only the 2nd half of the shape\n\n");
    printf("static float shapetab[2*SYMBLEN+1]=\n");
    printf("   {");
    for (int i = numcoeffs / 2; i < numcoeffs; i++) {
      if (i % 4 == 0 && i != 0) {
        printf("\n    ");
      }
      printf("%18.10e,", xcoeffs[i]);
    }
    printf("\n   };\n");
  } else {
    printf("G  = %18.10e\n", gain);
