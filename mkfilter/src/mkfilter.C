...
#define opt_R 0x20000 /* -R		sample rate                    */
#define opt_A 0x40000 /* -A		alpha*SAMPLERATE               */
...
static c_complex bessel_poles[] = {
    /* table produced by /usr/fisher/bessel --	N.B. only one member of each
       C.Conj. pair is listed */
...
static void readcmdline(char* argv[]) {
  ...
  until(argv[ap] == NULL) {
    ...
    if (m & opt_a) {
      ...
      raw_alpha2 = (argv[ap] != NULL && argv[ap][0] != '-')
                       ? getfarg(argv[ap++])
                       : raw_alpha1;
      raw_alpha2 = raw_alpha2;
    }
    // RR: I didn't like how the alpha argument to this program
    //     was already divided by the sample rate.  It made changing
    //     the sample rate difficult.  So I added two new options
    //     -R and -A so you can give the rate and the frequency
    //     separately.
    if (m & opt_R) {
      sample_rate = getfarg(argv[ap++]);
    }
    if (m & opt_A) {
      raw_alpha1 = getfarg(argv[ap++]);
      raw_alpha1 = raw_alpha1 / sample_rate;
      raw_alpha2 = (argv[ap] != NULL && argv[ap][0] != '-')
                       ? getfarg(argv[ap++])
                       : raw_alpha1;
      raw_alpha2 = raw_alpha2 / sample_rate;
    }
    if (m & opt_Z)
...
static void usage() {
  ...
  fprintf(stderr,
          "Usage: mkfilter [-Be | -Bu | -Ch <r> | -Pi] [-Lp | -Hp | -Bp | -Bs] "
          "[-p <n1> <n2> ...] [-{lwz}] "
          "[-Z <alphaz>] "
          "-o <order> -a <alpha1> [ <alpha2> ]\n");
  fprintf(stderr,
          "       mkfilter -Re <q> [-Bp | -Bs | -Ap] [-l] -a <alpha>\n\n");
  ...
  fprintf(stderr,
          "  -Lp, Hp, Bp, Bs, Ap = lowpass, highpass, bandpass, bandstop, "
          "allpass\n");
  fprintf(
      stderr,
      "  -p                  = use listed poles only (ni = 0 .. order-1)\n");
  ...
  if (options & opt_re) {
    ...
    if (options & opt_o) {
      ...
      if (options & opt_p) {
        ...
        if ((polemask & ~m) != 0)
          opterror("order=%d, so args to -p must be in range 0 .. %d", order,
                   order - 1);
      }
    } else
      opterror("must specify -o");
  }
  unless((options & opt_a) || (options & opt_A))
      opterror("must specify -a or -A");
...
static void
prewarp() { /* for bilinear transform, perform pre-warp on alpha values */
...
static void compute_z_blt() /* given S-plane poles & zeros, compute Z-plane
...
static void compute_z_mzt() /* given S-plane poles & zeros, compute Z-plane
                               poles & zeros, by matched z-transform */
{
  int i;
  zplane.numpoles = splane.numpoles;
  zplane.numzeros = splane.numzeros;
...
static void compute_notch() { /* compute Z-plane pole & zero positions for
                                 bandstop resonator (notch filter) */
...
static void compute_apres() { /* compute Z-plane pole & zero positions for
                                 allpass resonator */
  compute_bpres();            /* iterate to place poles */
...
static void compute_bpres() { /* compute Z-plane pole & zero positions for
                                 bandpass resonator */
  ...
  if (infq) {                        /* oscillator */
    ...
    for (int i = 0; i < 50 && !cvg; i++) {
      ...
      complex g = evaluate(topcoeffs, zplane.numzeros, botcoeffs,
                           zplane.numpoles, expj(theta));
...
static void expandpoly() /* given Z-plane poles & zeros, compute top & bot
                            polynomials in Z, and then recurrence relation */
{
  ...
  expand(zplane.poles, zplane.numpoles, botcoeffs);
  dc_gain =
      evaluate(topcoeffs, zplane.numzeros, botcoeffs, zplane.numpoles, 1.0);
  double theta =
      TWOPI * 0.5 * (raw_alpha1 + raw_alpha2); /* "jwT" for centre freq. */
...
static void expand(
    ...
    int npz,
    complex
        coeffs[]) { /* compute product of poles or zeros as a polynomial of z */
  int i;
  coeffs[0] = 1.0;
  for (i = 0; i < npz; i++)
    coeffs[i + 1] = 0.0;
...
static void multin(complex w,
                   int npz,
                   complex coeffs[]) { /* multiply factor (z-w) into coeffs */
  complex nw = -w;
  for (int i = npz; i >= 1; i--)
...
static void printresults(char* argv[]) {
  if (options & opt_l) { /* just list parameters */
    printcmdline(argv);
    complex gain = (options & opt_pi)
                       ? hf_gain
                       : (options & opt_lp)
                             ? dc_gain
                             : (options & opt_hp)
                                   ? hf_gain
                                   : (options & (opt_bp | opt_ap))
                                         ? fc_gain
                                         : (options & opt_bs)
                                               ? csqrt(dc_gain * hf_gain)
                                               : complex(1.0);
    printf("G  = %.10e\n", hypot(gain));
...
static void printrecurrence() /* given (real) Z-plane poles & zeros, compute &
